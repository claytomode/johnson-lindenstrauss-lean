...
| `qjlEstimator_concentration_exp` | `JL/QJLDistortion.lean` | `P(\|estimator − ⟨key/‖key‖,q⟩\| ≥ ε) ≤ 2·exp(−m·ε²/(π·‖q‖²))`, fully unconditional (no `key ≠ 0` needed). | **Same form as QJL Lemma 3** — exponential, `log(1/δ)` / `ε⁻²` rate, here with leading constant `π`. |
